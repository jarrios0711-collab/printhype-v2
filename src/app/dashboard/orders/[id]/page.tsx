      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-between items-end">
          ...
          <div className="flex gap-3">
              ...
              <Tooltip content="Enviar factura por WhatsApp al cliente">
                <a
                  href={(() => {
                    ...
                    const trackPath = order.trackingToken ? '/track/' + order.trackingToken : '/dashboard/orders/' + order.id
                    const invoiceUrl = window.location.origin + trackPath
                    const symbol = settings?.currency === 'USD' ? 'US$' : '$'
                    const msg = '🧾 *TU PEDIDO PrintHype - JR3D*\n\nCliente: ' + order.customerName + '\nTotal: ' + symbol + order.totalPrice.toLocaleString() + '\nEstado: ' + order.status + '\n\nSeguí tu pedido acá:\n' + invoiceUrl + '\n\n¡Gracias por confiar en JR3D! 🚀'
            ...
            {order.trackingToken && (
              <Tooltip content="Copiar el link público de seguimiento para el cliente">
                <button
                  onClick={() => {
                  ...
                  className="px-5 py-2.5 bg-brand-cyan/10 border border-brand-cyan/20 rounded-xl text-xs font-black hover:bg-brand-cyan/20 transition-all text-brand-cyan flex items-center gap-2"
                >
                  ...
                  {linkCopied ? '¡LINK COPIADO!' : 'LINK DE SEGUIMIENTO'}
                </button>
              </Tooltip>
            )}
